...
export const getPayoutMethod = async (req, res) => {
  try {
    ...
    return res.status(200).json(createExpressSuccessResponse('Payout method details retrieved successfully', sanitizedData));
...
export const createPayoutMethod = async (req, res) => {
  try {
    ...
    if (parseError) return res.status(400).json(createExpressErrorResponse('Invalid JSON in request body', 400));
    ...
    switch (type) {
      case 'paypal': {
        ...
        if (!validation.isValid) {
          return res.status(400).json(createExpressErrorResponse(validation.message, 400));
        }

        // Update user with PayPal configuration
        await updateUserPayoutMethod(userId, 'PayPal', '', validation.data.paypalEmail);

        return res.status(200).json(createExpressSuccessResponse('Changes saved successfully', {
          paypal: {
...
export const deletePayoutMethod = async (req, res) => {
  try {
    ...
    return res.status(200).json(createExpressSuccessResponse('Payout method deleted successfully'));
